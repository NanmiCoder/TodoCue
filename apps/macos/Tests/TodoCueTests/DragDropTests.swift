...
final class DragDropTests: XCTestCase {
    ...
    @MainActor func testOfflineCompletedAndPendingConfirmationCannotStartDragging() {
        XCTAssertFalse(AppModel().beginTaskDrag(task(), surface: .list(.all), group: "A"))
        let model = model()
        XCTAssertFalse(model.beginTaskDrag(task(status: .done), surface: .list(.all), group: "A"))
        model.pendingMove = PendingTaskMove(drag: TaskDrag(task: task(), surface: .list(.all), group: "A", revision: 0),
                                           target: TaskDropTarget(surface: .list(.all), group: "B", beforeId: nil))
        XCTAssertFalse(model.beginTaskDrag(task(), surface: .list(.all), group: "A"))
    }
    ...
    @MainActor func testTodayCrossGroupIsForbiddenAndEscapeCancelsWithoutClosingPanel() {
        ...
        XCTAssertTrue(model.beginTaskDrag(task(), surface: .list(.today), group: "2026-09-08:overdue"))
        let target = TaskDropTarget(surface: .list(.today), group: "2026-09-08:scheduled", beforeId: nil)
        model.dropTarget = target
        XCTAssertEqual(model.dragHint, L10n.tr("此分组由截止日期决定，请编辑日期"))
        XCTAssertFalse(model.dropTask(at: target))
        XCTAssertFalse(model.dropTask(at: TaskDropTarget(surface: .list(.all), group: "A", beforeId: nil)))
        model.handleEscape()
    ...
    @MainActor func testDropRegionsRejectOutsideBoundsAndResolveTopBottomAndEmptyGroup() {
        ...
        row.target = TaskDropTarget(surface: .list(.all), group: "A", beforeId: "first")
        row.afterId = "second"; row.isRow = true
        window.contentView!.addSubview(row)
        let outside = row.convert(NSPoint(x: 30, y: -30), to: nil)
        XCTAssertNil(TaskDropRegion.RegionView.target(at: outside, in: window, surface: .list(.all)))
        let top = row.convert(NSPoint(x: 30, y: 10), to: nil)
        let bottom = row.convert(NSPoint(x: 30, y: 40), to: nil)
        XCTAssertEqual(TaskDropRegion.RegionView.target(at: top, in: window, surface: .list(.all))?.beforeId, "first")
        XCTAssertEqual(TaskDropRegion.RegionView.target(at: bottom, in: window, surface: .list(.all))?.beforeId, "second")
        XCTAssertNil(TaskDropRegion.RegionView.target(at: top, in: window, surface: .list(.today)))
        row.enabled = false
        XCTAssertNil(TaskDropRegion.RegionView.target(at: top, in: window, surface: .list(.all)))
        row.enabled = true; row.isRow = false
        row.target = TaskDropTarget(surface: .list(.all), group: "", beforeId: nil)
        XCTAssertEqual(TaskDropRegion.RegionView.target(at: top, in: window, surface: .list(.all)), row.target)
    ...
    @MainActor func testClippedOffscreenRowsCannotReceiveADrop() {
        ...
        row.target = TaskDropTarget(surface: .list(.all), group: "A", beforeId: "hidden")
        document.addSubview(row)
        let point = row.convert(NSPoint(x: 20, y: 20), to: nil)
        XCTAssertNil(TaskDropRegion.RegionView.target(at: point, in: window, surface: .list(.all)))
    }
