...
final class DragMotionTests: XCTestCase {
    private func row(_ id: String, y: CGFloat, height: CGFloat = 48, group: String = "A", view: PanelTab = .all) -> TaskDragFrame {
        TaskDragFrame(slot: TaskDragSlot(view: view, group: group, kind: .row, taskID: id),
                      rect: CGRect(x: 0, y: y, width: 270, height: height))
    }
    private func end(y: CGFloat, group: String = "A", view: PanelTab = .all) -> TaskDragFrame {
        TaskDragFrame(slot: TaskDragSlot(view: view, group: group, kind: .end), rect: CGRect(x: 0, y: y, width: 270, height: 10))
    }
    ...
    func testMovingUpMakesExactlyTheInterveningRowsGiveWay() {
        ...
        let projection = TaskDragProjection.evaluate(frames: [a, b, c, d, end(y: 200)], source: c.slot,
                                                      target: TaskDropTarget(view: .all, group: "A", beforeId: "a"))
    ...
    func testMovingDownUsesDraggedRowHeightAndPreservesVariableHeightRows() {
        ...
        let projection = TaskDragProjection.evaluate(frames: [a, b, c, end(y: 181)], source: a.slot,
                                                      target: TaskDropTarget(view: .all, group: "A", beforeId: nil))
    ...
    func testReturningAcrossSameBoundaryIsReversibleAndDoesNotAccumulateOffsets() {
        ...
        let beforeA = TaskDropTarget(view: .all, group: "A", beforeId: "a")
        let first = TaskDragProjection.evaluate(frames: frames, source: b.slot, target: beforeA)
        _ = TaskDragProjection.evaluate(frames: frames, source: b.slot, target: TaskDropTarget(view: .all, group: "A", beforeId: nil))
        XCTAssertEqual(TaskDragProjection.evaluate(frames: frames, source: b.slot, target: beforeA), first)
        XCTAssertEqual(TaskDragProjection.evaluate(frames: frames, source: b.slot, target: TaskDropTarget(view: .all, group: "A", beforeId: "b")), TaskDragProjection())
    ...
    func testCrossProjectMovesIntermediateHeadersAndOpensAnEmptyGroup() {
        ...
        let header = TaskDragFrame(slot: TaskDragSlot(view: .all, group: "B", kind: .header), rect: CGRect(x: 0, y: 70, width: 270, height: 20))
    ...
    func testForbiddenTodayGroupNeverMakesSpace() {
        let a = row("a", y: 0, group: "overdue", view: .today)
        let b = row("b", y: 50, group: "scheduled", view: .today)
        XCTAssertEqual(TaskDragProjection.evaluate(frames: [a, b], source: a.slot,
                                                   target: TaskDropTarget(view: .today, group: "scheduled", beforeId: "b")), TaskDragProjection())
    }
    ...
    @MainActor func testTwoPointGutterDoesNotCollapseTheInsertionGap() {
        ...
        first.target = TaskDropTarget(view: .all, group: "A", beforeId: "a"); first.afterId = "b"; first.isRow = true
        let second = TaskDropRegion.RegionView(frame: NSRect(x: 0, y: 100, width: 270, height: 48))
        second.target = TaskDropTarget(view: .all, group: "A", beforeId: "b"); second.isRow = true
        window.contentView!.addSubview(first); window.contentView!.addSubview(second)
        XCTAssertEqual(TaskDropRegion.RegionView.target(at: NSPoint(x: 30, y: 149), in: window, view: .all)?.beforeId, "b")
        XCTAssertNil(TaskDropRegion.RegionView.target(at: NSPoint(x: 30, y: 80), in: window, view: .all))
    }
    ...
    @MainActor func testHostedPreviewKeepsMeasurementsFixedWhileRowsVisuallyGiveWay() async throws {
        ...
        let root = VStack(spacing: 2) {
            ...
            ForEach(Array(tasks.enumerated()), id: \.element.id) { index, task in
                DraggableTaskRow(task: task, view: .all, group: "工作", nextId: index < 2 ? tasks[index + 1].id : nil, showProject: false)
            }
        ...
        let before = TaskDropRegion.RegionView.frames(in: window, view: .all).sorted { $0.rect.minY < $1.rect.minY }
        ...
        XCTAssertTrue(model.beginTaskDrag(tasks[2], view: .all, group: "工作"))
        ...
        hosting.layoutSubtreeIfNeeded()
        let during = TaskDropRegion.RegionView.frames(in: window, view: .all).sorted { $0.rect.minY < $1.rect.minY }
    ...
    @MainActor func testPointerRemainsAttachedWhenScrollMovesUnderlyingRowsAndCancelRestoresThem() async throws {
        ...
        a.target = TaskDropTarget(view: .all, group: "A", beforeId: "a"); a.afterId = "b"; a.isRow = true
        let b = TaskDropRegion.RegionView(frame: NSRect(x: 0, y: 100, width: 270, height: 48))
        b.target = TaskDropTarget(view: .all, group: "A", beforeId: "b"); b.isRow = true
        ...
        XCTAssertTrue(model.beginTaskDrag(task, view: .all, group: "A"))
        ...
        XCTAssertEqual(presentation.projection.offsets[TaskDragSlot(view: .all, group: "A", kind: .row, taskID: "a")], 50)
